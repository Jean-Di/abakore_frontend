      ...
      <div className="relative hidden md:flex flex-col justify-between bg-navy-900 p-12 overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-8 flex flex-col gap-4">
            {[
              { icon: '⚖', title: '2 400+ experts vérifiés', body: 'Avocats, comptables, consultants certifiés dans tout l\'espace OHADA' },
              { icon: '🔒', title: 'Paiement 100% sécurisé', body: 'Système d\'escrow — vos fonds sont protégés jusqu\'à validation' },
              { icon: '🤖', title: 'IA OHADA intégrée', body: 'Posez vos questions juridiques et obtenez des réponses sourcées' },
            ].map(({ icon, title, body }) => (
              <div key={title} className="flex gap-3">
                <div className="w-8 h-8 flex-shrink-0 rounded-lg bg-gold-500/12 border border-gold-500/20 flex items-center justify-center text-sm">{icon}</div>
                <div>
        ...
        <p className="relative z-10 text-[11px] text-white/20">© 2024 Abakoré · Droit OHADA / UEMOA</p>
      </div>
      ...
      <div className="flex items-center justify-center bg-white p-8">
        <div className="w-full max-w-sm">
          ...
          <form className="space-y-4">
            ...
            <button type="submit" className="btn-gold w-full justify-center py-3 text-sm rounded-xl">Connexion →</button>
          </form>
