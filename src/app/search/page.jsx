...
function useExpertSearch({ verifiedOnly, checkedCountries, sortBy, activeType }) {
  const [experts, setExperts] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)
  ...
  const fetchExperts = useCallback(async () => {
    ...
    try {
      ...
      // ── Role selon le type sélectionné ─────────────────────
      if (activeType !== null) {

        const selectedType = EXPERT_TYPES[activeType]
console.log("selectedType", selectedType)
        // Exemple :
        // Avocats     => role = "avocat"
        // Comptables  => role = "comptable"
        // RH          => role = "rh"
        ...
        const role = roleMap[selectedType?.label]
console.log("role", role)
      ...
      if (checkedCountries.length === 1) {
        constraints.push(where('country', '==', checkedCountries[0]))
      }

      // ── Tri ────────────────────────────────────────────────
      // Je recommande le tri côté client pour éviter les exclusions Firestore
  ...
  }, [verifiedOnly, checkedCountries, sortBy, activeType])
...
export default function SearchPage() {
  ...
  const { experts, loading, error } = useExpertSearch({
  verifiedOnly,
  checkedCountries,
  sortBy,
  activeType,
